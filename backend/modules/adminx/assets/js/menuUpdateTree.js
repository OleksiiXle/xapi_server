...
var MENU_TREE = {
    ...
    clickIcon: function (parent) {
        ...
        switch (parent.innerHTML){
            case ICON_CLOSE:
                ...
                $.ajax({
                    url: '/adminx/menux/menux-get-children',
                    type: "POST",
                    data: {
                        'id' : parent_id,
                        '_csrf':_csrfT
    ...
    //    alert(this.tree_id + ' modalOpenMenuUpdate ' + this.selected_id);
        ...
        var url = '/adminx/menux/menux-modal-open-menu-update';
        var data = {
            'id' : that.selected_id,
        ...
        $.ajax({
           ...
           // dataType: 'json',
            ...
            success: function(response){
                if ($('#main-modal-md').length > 0){
                    $('#main-modal-md').modal('show').find('.modal-content').html(response);
                } else{
                    var mHeader;
                    switch (nodeAction) {
                        case 'update':
                            ...
                            break;
                        case 'appendChild':
                            mHeader = 'Добавить потомка';
                            break;
                        case 'appendBrother':
                            mHeader = 'Добавить брата вниз';
                            break;


                    }
                    showModal(600,550, mHeader);
    ...
    menuUpdate: function () {
        ...
        var new_item;
        $.ajax({
            url: '/adminx/menux/menux-menu-update',
            type: "POST",
            data: data,
            dataType: 'json',
            beforeSend: function() {
...
sort: 2
    ...
    treeModifyAuto : function (nodeAction) {
      ...
      //  console.log(node1_li);
        ...
        $.ajax({
            url: '/adminx/menux/menux-tree-modify-auto',
            type: "POST",
            data: {
                'node1_id' : node1_id,
                'node2_id' : node2_id,
